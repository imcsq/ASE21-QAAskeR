...
class when(object):
    ...
    def generate_statement(self, question, answer):
        ...
        if vbs == []:
            ...
            if tag_tokens_answer[0] in ["IN"] and tag_tokens_new_question[when_plc[0] - 1] not in ["IN"]:
                final.extend([answer])
            elif tag_tokens_answer[0] not in ["IN"] and tag_tokens_new_question[when_plc[0] - 1] in ["IN"]:
                final.extend([answer])
            elif tag_tokens_answer[0] in ["IN"] and tag_tokens_new_question[when_plc[0] - 1] in ["IN"]:
                final.extend(str_tokens_answer[1:])
            else:
                final.extend(["in", answer])
            final.extend(str_tokens_new_question[when_plc[0] + 1:])
            final.pop()
        ...
        if str_tokens_new_question[-2] == "when":
            final = []
            final.extend(str_tokens_new_question[:when_plc[0]])
            if tag_tokens_answer[0] in ["IN"] and tag_tokens_new_question[when_plc[0] - 1] not in ["IN"]:
                final.extend([answer])
        ...
        if first_vb in be:
            if str_tokens_new_question[0] in ["When", "when"]:
                ...
                if pos_tokens_output2[root_plc] == "VERB":
                    final = []
                    if lemma_tokens_output2[0:3] == ["the", "last", "time"]:
                        final.extend(str_tokens_new_question[first_vb_plc + 1:first_vb_plc + 4])
                        final.extend(str_tokens_new_question[first_vb_plc + 4:])
                        final.pop()
                        ...
                        if have_comma_do:
                            ...
                            final_out = final_out[0].upper() + final_out[1:] + "."
                        return final_out
                    if pos_tokens_new_question[len(output1) + root_plc - 2] in ["ADJ", "ADV", "ADJP"] and \
                            pos_tokens_new_question[len(output1) + root_plc - 1] in ["ADJ", "ADV", "ADJP"]:
                        final.extend(str_tokens_new_question[first_vb_plc + 1:len(output1) + root_plc - 2])
                        final.extend([first_vb])
                        final.extend(str_tokens_new_question[len(output1) + root_plc - 2:])
                        after_when = str_tokens_new_question[when_plc[0] + 1:first_vb_plc]
                        final.pop()
                        if tag_tokens_answer[0] in ["IN"]:
                            final.extend([answer])
                        else:
                            final.extend(["in", answer])
                        final.extend(after_when)
                        final_out = list_to_str(final)
                        if have_comma_do:
                            final_out = final_add + ", " + final_out + "."
                        else:
                            final_out = final_out[0].upper() + final_out[1:] + "."
                        return final_out
                    if pos_tokens_new_question[len(output1) + root_plc - 1] in ["ADJ", "ADV", "ADJP"]:
                ...
                if tag_tokens_output2[-2] in ["IN"]:
                    ...
                    return final_out
                have_verb = False
                tag = nltk.pos_tag(next_lemma_tokens_output2)
                nltk_tag = [this_tag[1] for this_tag in tag]
                for i in nltk_tag:
                    if "VB" in i:
                        have_verb = True
                        next_root = i
                        next_root_plc = nltk_tag.index(i)
                if have_verb:
                    final = []
                    final.extend(
                        str_tokens_new_question[first_vb_plc + 1:len(output1) + len(next_output1) + next_root_plc])
                ...
                if "VERB" not in pos_tokens_output2:
                    ...
                    final.pop()
                    final.extend([first_vb])
                    before_when = str_tokens_new_question[:when_plc[0]]
                    before_when[0] = before_when[0][0].lower() + before_when[0][1:]
                    final.extend(before_when)
                    final.extend([answer])
                    final_out = list_to_str(final)
                    if have_comma_do:
                        final_out = final_add + ", " + final_out + "."
                ...
                if pos_tokens_output2[root_plc] == "VERB":
                    ...
                    if pos_tokens_new_question[first_vb_plc + root_plc] == "ADV" and \
                        ...
                        final.extend(str_tokens_new_question[first_vb_plc + 1:first_vb_plc + root_plc - 2])
                        final.extend([first_vb])
            ...
            if str_tokens_new_question[0] in ["When", "when"]:
                ...
                if pos_tokens_output2[root_plc] == "VERB" or root_is_vb:
                    if first_vb in did:
                        ...
                        if len(tag_tokens_answer) == 0:
                            final = final
                        elif tag_tokens_answer[0] in ["IN"] and tag_tokens_new_question[-2] not in ["IN"]:
                            final.extend([answer])
                        elif tag_tokens_answer[0] not in ["IN"] and tag_tokens_new_question[-2] in ["IN"]:
                            final.extend([answer])
                        elif tag_tokens_answer[0] in ["IN"] and tag_tokens_new_question[-2] in ["IN"]:
