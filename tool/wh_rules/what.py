    ...
    def generate(self, question, answer):
        ...
        for token in tokens_question:
            if token.string.strip() == ",":
                ...
                have_comma = True
            if token.string.strip() == "what" or token.string.strip() == "What":
                all_what.append(num)
            num += 1
        ...
        if vbs == []:
            if str_tokens_new_question[0] in ["What", "what"]:
                ...
                tag_what_tree = [token.tag_ for token in doc_what_tree if token.string.strip() != ""]
                num = 0
        ...
        if first_vb in do:
            if str_tokens_new_question[0] in ["What", "what"]:
                ...
                str_tokens_output1 = [token.string.strip() for token in doc_output1 if token.string.strip() != ""]
                tag_tokens_output1 = [token.tag_ for token in doc_output1 if token.string.strip() != ""]
                ...
                if need_change:
                    if first_vb in would:
                        ...
                        if root_plc == 0:
                            output1 = str_tokens_new_question[:first_vb_plc]
                            doc_output1 = nlp(list_to_str(output1))
                            str_tokens_output1 = [token.string.strip() for token in doc_output1 if
                                                  token.string.strip() != ""]
                            tag_tokens_output1 = [token.tag_ for token in doc_output1 if token.string.strip() != ""]
                            vb = str_tokens_new_question[first_vb_plc]
                            ...
                            doc_output2 = nlp(list_to_str(output2))
                            tokens_output2 = [token for token in doc_output2 if token.string.strip() != ""]
                            str_tokens_output2 = [token.string.strip() for token in doc_output2 if
                                                  token.string.strip() != ""]
                            dep_tokens_output2 = [token.dep_ for token in doc_output2 if token.string.strip() != ""]
                            pos_tokens_output2 = [token.pos_ for token in doc_output2 if token.string.strip() != ""]
                            root = str_tokens_output2[dep_tokens_output2.index("ROOT")]
                            ...
                            if "VERB" in pos_tokens_new_question and pos_tokens_output2[1] != "VERB" and "VB" not in \
                                ...
                                for pos in pos_tokens_new_question[:-1]:
                                    ...
                                    if pos in ["ADJ", "ADV", "ADJP"] and str_tokens_new_question[num + 1] == "to":
                                        ...
                                        tokens_output3 = [token for token in doc_output3 if token.string.strip() != ""]
                                        str_tokens_output3 = [token.string.strip() for token in doc_output3 if
                                                              token.string.strip() != ""]
                ...
                if (len(IN) + len(WHAT) + 1) == len(str_tokens_new_question) or vbs_plc[0] < what_plc[0]:
                    ...
                    if root in be:
                        ...
                        if len(acl_plc):
                            ...
                            str_tokens_output3 = [token.string.strip() for token in doc_output3 if
                                                  token.string.strip() != ""]
                            dep_tokens_output3 = [token.dep_ for token in doc_output3 if token.string.strip() != ""]
                            dep_tokens_output3 = [token.dep_ for token in doc_output3 if token.string.strip() != ""]
                            pos_tokens_output3 = [token.pos_ for token in doc_output3 if token.string.strip() != ""]
