    ...
    def generate(self, question, answer):
        ...
        for token in tokens_question:
            if token.string.strip() == ",":
                ...
                have_comma = True
            if token.string.strip() == "Why" or token.string.strip() == "why":
                all_why.append(num)
            num += 1
        ...
        if vbs == []:
            ...
            num = 0
            for pos in pos_tokens_new_question:
                if pos == "VERB" and tokens_new_question[num].string.strip() != conjugate(
                    ...
                    if pos_tokens_output2[root_plc] == "VERB":
                        ...
                        for child in children:
                            if child.pos_ in ["ADJ", "ADV", "ADJP"] and pos_tokens_output2.index(child.pos_) > root_plc:
                                root_plc = str_tokens_output2.index(child.string.strip())
                                final = []
